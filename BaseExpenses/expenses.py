...
Builder.load_file('BaseExpenses/expenses.kv')
...
class ExpensesWindow(BoxLayout):
    ...
    def export_dated(self):
        if not self.dated_already:
            ...
            try:
                ...
                datatoExcel = pd.ExcelWriter("C:\CocabTechSolutionsPos\Dated_Expenses_Search.xlsx",
                                             engine='xlsxwriter')
                ...
                self.showAlert("Export was successful")
    ...
    def export_all_additions(self):
        ...
        try:
            ref = overall.dbs.reference('expenses')
            snapshot = ref.get()

            if snapshot:
                additions = snapshot.values()
                data = pd.DataFrame(additions)
                datatoExcel = pd.ExcelWriter("C:\CocabTechSolutionsPos\Expenses_All.xlsx", engine='xlsxwriter')
                data.to_excel(datatoExcel, sheet_name='Sheet1')
                datatoExcel.save()
    ...
    def fectchall(self):
        ...
        try:
            ...
            if snapshot:
                ...
                total = str(sum(the_total_search))
                self.ids.expenses_text.text = "Overal Expenses:             Total Expenses:     KES:    " + total

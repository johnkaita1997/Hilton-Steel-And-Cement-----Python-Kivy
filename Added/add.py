...
Builder.load_file('Added/add.kv')
    ...
    def export_dated(self):
        try:
            if not self.dated_already:
                ...
                datatoExcel = pd.ExcelWriter("C:\CocabTechSolutionsPos\Dated_Additions_Search.xlsx",
                                             engine='xlsxwriter')
    ...
    def dated_Search(self):
        ...
        try:
            ...
            try:
                ...
                path = overall.dbs.reference("additionshistory")
                snapshot = path.order_by_child('A-Date').get()
                ...
                if snapshot:
                    for sale in snapshot.values():
                        ...
                        if editeddate <= editedmaxdate and editeddate >= editedmindate:
                            retrieve_amount = sale['A-Total-Sales']
                    ...
                    self.ids.thesales.text = "Total Sales For this period " + total
    ...
    def export_all_additions(self):
        ...
        try:
            ...
            if snapshot:
                ...
                data = pd.DataFrame(additions)
                datatoExcel = pd.ExcelWriter("C:\CocabTechSolutionsPos\All_Additions.xlsx", engine='xlsxwriter')
                data.to_excel(datatoExcel, sheet_name='Sheet1')
                datatoExcel.save()
    ...
    def search_Id(self):
        try:
            ...
            if not inp_idnuimber:
                ...
                ref = overall.dbs.reference('additionshistory')
                snapshot = ref.order_by_child('Didnumber').equal_to(inp_idnuimber).get()
        ...
        try:
            ...
            if snapshot:
                for value in snapshot.values():
                    ...
                    thelist.append(john)
                    retrieve_amount = value['A-Total-Sales']
                    the_total_search.append(float(retrieve_amount))

                self.data_items = []
                self.data_items = thelist
                total = str(sum(the_total_search))
                self.ids.thesales.text = "The total sales is " + total
